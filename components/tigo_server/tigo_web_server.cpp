...
void TigoWebServer::build_devices_json(PSRAMString& json) {
  ...
  for (const auto &dwn : sorted_devices) {
    ...
    if (dwn.has_runtime_data && dwn.device != nullptr) {
      ...
      float effective_voltage = parent_->get_effective_voltage(device.voltage_in, device.voltage_out);
      float power = effective_voltage * device.current_in * parent_->get_power_calibration();
